"""Single-axis weakness probe — VECTOR-POINTING variant of the ALTRO
spike-removal stack.

Each scenario changes ONE thing from the baseline and saves:
    wide_results_vec/<name>.gif          - convergence animation
    wide_results_vec/<name>_final.png    - final-iteration 4-panel
    wide_results_vec/<name>_midway.png   - progression of key iterations

Env vars:
    WIDE_COSTREF=1            — enable α-from-β crossterm + axis-aware W_ω.
                                Output → wide_results_vec_costref/.
    WIDE_COSTREF_BETA=<f>     — β value (default 0.3).
    WIDE_ROLL_RATIO=<f>       — α_roll for axis-aware W_ω (default 0.05).
"""
...
ROLL_RATIO = float(os.environ.get("WIDE_ROLL_RATIO", "0.05"))
...
default_out = "wide_results_vec_costref" if COSTREF_ENABLED else "wide_results_vec"
...
def baseline_params():
    return dict(
        ...
        disturbances={},
        use_spike=True,
        max_iters=int(__import__("os").environ.get("WIDE_MAX_ITERS", "200")),
        max_outer=int(__import__("os").environ.get("WIDE_MAX_OUTER", "30")),
    )
...
def build_planner(p):
    ...
    if COSTREF_ENABLED:
        # New path: PSD-bounded α·err_dir·(ω−ω_ff) crossterm via β. Vector mode
        # uses err_dir = (R^T r̂) × bs and λ_min(W_ω) = ang_vel·roll_ratio.
        c.ang_vel_err_dir = 0.0
        c.ang_vel_err_dir_ratio = COSTREF_BETA
        c.ang_vel_roll_ratio = ROLL_RATIO
    else:
        # Legacy path: directional ω cost via raw `ang_vel_err_dir` coefficient.
    ...
    # reg_init = 1e-12 per 2026-04-24 principled-defaults change (was 1e-6 here,
    # which over-regularized Q_uu from step 1 regardless of problem structure).
...
def run_scenario(name, params):
    ...
    # VECTOR-POINTING goal (2-DOF constraint; no roll). qgoal's first slot is
    # NaN to signal ECI-vector-pointing mode; remaining 3 slots are the ECI
    # direction that body boresight `bs` should align with.
    # For a Z-axis rotation of `goal_angle_deg`, body-X (our boresight) ends up at
    # ECI vector (cos θ, sin θ, 0). So a 90° slew → (0, 1, 0); 180° → (-1, 0, 0).
    ...
    if params["use_spike"]:
        cfg = {
            ...
            # gate_thresh = constraint_gate_ratio × constraint_tol; skip when
            # max_c < gate_thresh. ratio=0 ⇒ never skip.
    ...
    # Metrics — use mode-aware pe_profile so vector targets (qg[0]=NaN)
    # don't produce NaN. The legacy inline formula here was the source of
    # the spurious "PE_fin = NaN" in summary output.
    pe = pe_profile(X, qg)
    q = X[3:7, :]; N = q.shape[1]
...
def _quat_rotmat(q):
    """Active rotation matrix R(q) (Hamilton convention)."""
    q0, qv = q[0], q[1:]
    qx, qy, qz = qv
...
def pe_profile(X, qg, bs=np.array([1.0, 0.0, 0.0])):
    """Pointing-error profile.  For vector-pointing mode (qg[0] is NaN), PE is
    the angle between body boresight `bs` rotated to ECI and the target ECI
    direction `qg[1:]`. For quaternion mode (qg[0] finite), PE is the
    quaternion-distance angle as before."""
    if not np.isfinite(qg[0]):
        r_eci = qg[1:] / max(np.linalg.norm(qg[1:]), 1e-12)
        bs_unit = bs / max(np.linalg.norm(bs), 1e-12)
...
if COSTREF_ENABLED:
    print(f"Cost refactor (vec): β={COSTREF_BETA}, roll_ratio={ROLL_RATIO}")

...
def run_ilqr_python_loop(satellite, settings, X0, U0, jtime, R, V, B, S, rho, boresight, attitude_target_traj):
    ...
    for _ in range(ilqr_cfg.max_iters):
        ...
        try:
            ...
            break
        if not ok_fp:
            stop_reason = "forward pass / line search failed"
            break
...
def launch_iteration_viewer(snapshots, transitions, stop_reason, dt, cost_tol):
    ...
    def update_view(i):
        ...
        if i > 0 and i - 1 < len(transitions):
            tr = transitions[i - 1]
            lines.append("")
            lines.append("Current selected-step checks:")
            lines.append(f"predicted ΔJ: {tr['pred_delta']:.6e}")
            lines.append(f"actual ΔJ:    {tr['act_delta']:.6e}")
            lines.append(f"bp_ok: {tr['bp_ok']} | fp_ok: {tr['fp_ok']}")
            lines.append(f"J decrease achieved: {tr['cost_decrease_ok']}")
            lines.append(f"|ΔJ| <= tol achieved: {tr['delta_tol_ok']}")

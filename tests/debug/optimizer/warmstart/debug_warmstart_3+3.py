...
def setup_satellite(init_controller: int = 2):
    """Create 3 MTQ + 3 RW satellite and planner settings."""
...
def print_summary(X: np.ndarray, U: np.ndarray, ws_time: float):
	...
	rw_u = U[3:6, :] if U.shape[0] >= 6 else np.zeros((0, U.shape[1]))
	...
	if rw_u.size > 0:
		print("RW torque usage [N m] (min / mean / max abs per axis):")
		for i in range(rw_u.shape[0]):
			ua = np.abs(rw_u[i, :])
			print(f"  rw{i}: {ua.min():.6f} / {ua.mean():.6f} / {ua.max():.6f}")


def plot_results(X: np.ndarray, U: np.ndarray, dt: float):
	"""Plot quaternion, angular rates, MTQ control, and RW control."""
	N = X.shape[1]
	...
	fig.suptitle("Warm-start Controller Test (3+3)", fontsize=13)
	if matplotlib.get_backend().lower().endswith("agg"):
		out_path = Path(__file__).with_name("debug_warmstart_3+3_plots.png")

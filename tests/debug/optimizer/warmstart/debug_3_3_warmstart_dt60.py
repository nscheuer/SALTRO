"""Simple warm-start controller test (3 MTQ + 3 RW)."""
...
import saltro_py
from create_3_3_sat import create_3_3_satellite
...
def setup_planner_settings(init_controller: int = 2, dt: float = 60.0):
...
def print_summary(X: np.ndarray, U: np.ndarray, ws_time: float):
    ...
    num_mtq = min(3, U.shape[0])
    mtq_u = U[0:num_mtq, :] if num_mtq > 0 else np.zeros((0, U.shape[1]))
    if mtq_u.size > 0:
        print("MTQ usage [A m^2] (min / mean / max abs per axis):")
        for i in range(mtq_u.shape[0]):
            ua = np.abs(mtq_u[i, :])
            print(f"  mtq{i}: {ua.min():.4f} / {ua.mean():.4f} / {ua.max():.4f}")

    rw_u = U[num_mtq : num_mtq + 3, :] if U.shape[0] >= num_mtq + 1 else np.zeros((0, U.shape[1]))
    if rw_u.size > 0:
...
def main():
    ...
    satellite = create_3_3_satellite(settings)
    ...
    launch_viewer(X, U, dt, satellite, title="Warm-start: 3 MTQ + 3 RW Detumble (dt60)")

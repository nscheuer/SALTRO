"""
Verify: eigenvalue-clamping lxx to PSD fixes backward pass for all trajectory lengths.
"""
import sys
import numpy as np
...
def clamp_psd(M):
    """Eigenvalue-clamp symmetric matrix to PSD."""
    eigvals, eigvecs = np.linalg.eigh(M)
    if eigvals[0] >= 0:
        return M  # Already PSD
...
def run_backward_pass(total_time_sec, reg, clamp_lxx):
...
def main():
    reg = 1e-6  # Default low reg
    total_times = [200, 220, 300, 500, 1000, 2000, 5000]

    print("=" * 100)
    print("lxx PSD clamping test (reg=1e-6)")
    print("=" * 100)
    print(f"{'time':>6} | {'No clamp':>25} | {'With clamp':>25} | {'P_k min eig':>14} | {'Q_uu min eig':>14} | {'max||P||':>12}")
    print("-" * 100)

    for t in total_times:
        fail_no, N, _, minP_no, minQ_no = run_backward_pass(t, reg, clamp_lxx=False)
        fail_yes, _, maxPk, minP_yes, minQ_yes = run_backward_pass(t, reg, clamp_lxx=True)

        no_str = f"FAIL k={fail_no}" if fail_no >= 0 else f"OK ({N-1} steps)"
        yes_str = f"FAIL k={fail_yes}" if fail_yes >= 0 else f"OK ({N-1} steps)"
        print(f"{t:6d} | {no_str:>25} | {yes_str:>25} | {minP_yes:14.4e} | {minQ_yes:14.4e} | {maxPk:12.4e}")

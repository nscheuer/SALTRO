"""
Verify the complete fix: lxx PSD clamping + regularized Q_uu for P_k update.
"""
import sys
import numpy as np
...
def run_backward_pass(total_time_sec, reg, clamp_lxx, use_reg_for_pk):
    ...
    for k in range(N-2, -1, -1):
        ...
        Q_for_Pk = Q_uu_reg if use_reg_for_pk else Q_uu
        P_k = Q_xx + K_k.T @ Q_for_Pk @ K_k + K_k.T @ Q_ux + Q_ux.T @ K_k
        p_k = Q_x + K_k.T @ Q_for_Pk @ d_k + K_k.T @ Q_u + Q_ux.T @ d_k
...
def main():
    ...
    configs = [
        ("No fix",        False, False),
        ("Clamp only",    True,  False),
        ("Reg P_k only",  False, True),
        ("Clamp + Reg",   True,  True),
    ]
    
    print("=" * 120)
    print(f"Comparison of fixes (reg={reg:.0e})")
    print("=" * 120)
    
    for label, clamp, reg_pk in configs:
        print(f"\n--- {label} ---")
        print(f"{'time':>7} | {'result':>20} | {'min P_k eig':>14} | {'min Q_uu eig':>14} | {'max ||P||':>12}")
        print("-" * 75)
        for t in total_times:
            fail, N, maxP, minP, minQ = run_backward_pass(t, reg, clamp, reg_pk)

...
def test_igrf13_dimensions_and_validity():
    N = 5
    Re = 6378136.3
    ...
    ok, B = saltro_py.compute_magnetic_igrf13(R, jtime)
...
def test_igrf13_circular_orbit_smoothness():
    N = 60
    Re = 6378136.3
    alt = 500e3
    r = Re + alt
...
def test_igrf13_input_validation():
    R_bad = np.zeros((2, 5))
    jtime = np.ones(5)

    with pytest.raises(RuntimeError):
        saltro_py.compute_magnetic_igrf13(R_bad, jtime)

    R = np.zeros((3, 5))
    jtime_bad = np.ones(4)

    with pytest.raises(RuntimeError):
        saltro_py.compute_magnetic_igrf13(R, jtime_bad)